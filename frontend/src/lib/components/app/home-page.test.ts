...
function parseCardData(input: string) {
  ...
  return blocks.map((block, index) => {
    ...
    const readFieldValue = (line: string, prefix: string) =>
    ...
    for (const line of lines) {
      ...
      if (line.startsWith("QUESTION_IMAGE::")) {
        questionImages.push({
          id: readFieldValue(line, "QUESTION_IMAGE::"),
          mimeType: "image/png",
          dataBase64: "aGVsbG8=",
        });
        continue;
      ...
      if (line.startsWith("ANSWER_IMAGE::")) {
        answerImages.push({
          id: readFieldValue(line, "ANSWER_IMAGE::"),
          mimeType: "image/png",
          dataBase64: "aGVsbG8=",
        });
        continue;
      ...
      throw new Error(
        `Карточка ${index + 1} содержит строку в неверном формате.`,
      );
    }
...
describe("HomePage cards list preview", () => {
  it("keeps the last question outside the visible list viewport for 50 cards", async () => {
    ...
    await fireEvent.click(
      screen.getByRole("button", { name: "Показать список карточек" }),
    );
    ...
    expect(lastCard.getBoundingClientRect().top).toBeGreaterThan(
  ...
  it("deletes a card only after second click within 4 seconds", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Подтвердить удаление карточки" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getAllByRole("button", { name: "Удалить карточку" }),
    ).toHaveLength(1);
  });
  ...
  it("cancels delete confirmation after 4 seconds", async () => {
    ...
    await fireEvent.click(
      screen.getAllByRole("button", { name: "Удалить карточку" })[0],
    );

    await vi.advanceTimersByTimeAsync(4000);

    expect(
      screen.queryByRole("button", { name: "Подтвердить удаление карточки" }),
